...
def registerUser(request):
    ...
    if request.method =="POST":
        ...
        if password1 == password2:
            if not User.objects.filter(username=username).exists(): #db de inputtan girilen kullanıcı adı yoksa kayıt olabilir.
                if not User.objects.filter(email=email).exists():
                    ...
                    context.update({"hata":"Bu Email adresi daha önce kullanılmış.."})
            else:
                    context.update({"hata":"Bu Kullanıcı Adı daha önce kullanılmış.."})
        else:
                    context.update({"hata":"Şifreleri tekrar kontrol edin.."})
